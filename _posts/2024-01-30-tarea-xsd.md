...
Dada la definición de `a09_vehiculo.xsd.txt`{: .filepath} escribe otro modo de definir el elemento "vehiculo" y un `xs:simleType` llamado `tipoDeVehiculo` que restringiese a "barco", "bicicleta", "coche" y "tren" como los únicos valores aceptables para el vehículo, de forma que dicho tipo pudiera ser también utilizado por otros elementos.
...
En `a12_respuestasAdmitidas.xsd.txt`{: .filepath} se define un elemento llamado "respuesta" con la restricción de que el único valor aceptable es una de las siguientes letras: "A", "B", "C", "D" o "E". Escribe otra forma de especificar la misma restricción.
